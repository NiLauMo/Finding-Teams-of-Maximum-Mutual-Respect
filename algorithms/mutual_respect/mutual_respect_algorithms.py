...
# 算indegree(max mutual respect會用到)
def compute_indegree(graph,weighted):
	...
	for author, indegree in author_indegree.items():
...
def get_authors_activity_per_graph(category_graph,kcore):
	...
	authors_set = list(set(authors_list))      # set 會去掉重複元素而且排序
                                               # 所以會是一個排序的數列 
...
def get_all_statistics(category_graph):
	kcore = 1
	num_nodes = []; num_edges = []
	for category, DG in category_graph.items():
...
# 算outdegree跟indegree，最後回傳該作者的總得分
def compute_indegree_outdegree(main_category, category_graph, weighted):
	...
	# avg outdegree
	for node, outdegree in author_score.items():
...
def graph_random(category_graph,weighted):
	...
	while counter < 200:
		...
		while category_graphs_copy:
			...
			category = random.choice(list(category_graphs_copy.keys()))
...
def greedy_algorithm(category_rankings, weighted):
	...
	for outer_category, outer_category_ranking in category_rankings.items():
		for outer_ranking, outer_author_score in enumerate(outer_category_ranking):
			...
			while category_rankings_copy:
				...
				for inner_category, inner_category_ranking in category_rankings_copy.items():
				...
				selected_author_score_category = random.choice(list(same_highest_score_solutions))
...
def greedy_algorithm_random_skills(category_rankings, weighted):
	...
	for outer_category, outer_category_ranking in category_rankings.items():
		for outer_ranking, outer_author_score in enumerate(outer_category_ranking):
			...
			while category_rankings_copy:
				highest_score = 0
				inner_category = random.choice(list(category_rankings_copy.keys()))
...
def greedy_algorithm_random_skills_stop(category_rankings, weighted, skill_author_index_dict):
	...
	for outer_category, outer_category_ranking in category_rankings.items():
		...
		for outer_ranking, outer_author_score in enumerate(outer_category_ranking):
			...
			for category,value in seen_skills_author_inner.items():
				if category != outer_category:
					seen_skills_author_inner[category] = {}

			for category,value in seen_skills_author.items():
				if category != outer_category:
...
def compute_graph_score(solution_dict,category_graph):
	...
	# 回傳分數(mutual respect的總和)跟存在的邊(有關連的)
...
def store_category_ranks(category_ranks,store,kcore):
	...
	if store == True:
		for category, ranking in category_ranks.items():
			try:
				category_name_list = category.split(' ')
...
# 算分數，如果分數(indegree+ outdegree)比較大就用它
def find_random_solution(indegree_solution_dict, outdegree_solution_dict, outdegree_dict, graphs, solution_dict,selected_category_dict,selected_node_dict):
	...
	while assigned_nodes < len(graphs):
		max_score = -float('inf')
		# print('New assignment.')
		category = random.choice(list(graphs_copy.keys()))
...
def find_first_assignments(indegree_per_category_dict,outdegree_dict,graphs_copy):
	...
	for category, graph in graphs_copy.items():
		for edge in graph.edges():
			from_node = edge[0]; to_node = edge[1]
	...
	for indegree_key,indegree_value in indegree_per_category_dict.items():
	...
	for assignment, score in score_dict.items():
...
def graph_algorithm_random(graphs,weighted):
	...
	while counter < 10:
		start = time.time()
		solution_dict = {}
		random_category = random.choice(list(graphs_copy.keys()))
		...
		solution_dict[best_category] = (best_node,max_score) #存成dict
