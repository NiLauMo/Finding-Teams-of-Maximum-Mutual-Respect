...
def jaccard_similarity_score(category_graph):
	...
	for key,alist in jaccard_similarity_score_lists.items():
...
#	# uncomment to read graphs from files 
	for category, graph in category_graph.items():
		ebunch = nx.selfloop_edges(graph) # 回傳自己的迴圈 ex{1,1}
		graph.remove_edges_from(ebunch)
		print('Creating core graph.')
		core_graph = nx.k_core(graph,k=k_core) # 回傳>=k_core的子圖
		if weighted == False:
...
def half_approximation_algorithm(category_graph_rankings, weighted):
	...
	while category_graph_rankings:
		...
		for category, category_ranking in category_graph_rankings.items():
...
def main(args):
	...
	for customized_categories in customized_categories_list:
		...
		category_graph_clean = clean_graphs(category_graph,kcore,weighted) # k=5, weight=false
		...
		for category, graph in category_graph_clean.items():
			...
			nd = sorted(dict(graph.degree).items(), key=lambda x: x[1],reverse=True)[0][0]
