...
def main(args):
	...
	not_included_dict = {'Player':1, 'Year':1, 'Pos':1, 'Age':1, 'Team': 1, 'Unnamed: 0':1, 'blanl':1, 'blank2':1, 'GS Games Started':1, 'MP Minutes Played':1,\
						...
						'DRB':1,'TRB':1,'AST':1,'STL':1,'BLK':1,'TOV':1,'PF':1, 'MPG':1
						}
	...
	included_dict = {'3P':1,'TRB':1,'AST':1,'STL':1,'BLK':1,'FG':1,'2P':1,'FT':1}


	file_path = 'WNBA_Stats.csv'
	...
	for year in range(2018,2020,1):
		...
		df = df.groupby(['Player','G']).mean().reset_index() #AGE
		df = df.loc[df['G'] >= 34]	#>=27 #>=15
		df = df.loc[df['MPG'] >= 18] #>=15	#>=10
		df = df.fillna(0)
		# df = df.head(60)
